...
*/
#define DLOPEN_LIBMMCAMERA 1
#define DUMP_VIDEO_FRAMES 1
//#define LOG_NDEBUG 0
...
static const str_map whitebalance[] = {
    ...
    { WHITE_BALANCE_INCANDESCENT,    CAMERA_WB_INCANDESCENT },
    { WHITE_BALANCE_FLUORESCENT,     CAMERA_WB_FLUORESCENT },
    { WHITE_BALANCE_DAYLIGHT,        CAMERA_WB_DAYLIGHT },
    { WHITE_BALANCE_CLOUDY_DAYLIGHT, CAMERA_WB_CLOUDY_DAYLIGHT }
};

// from camera_effect_t. This list must match aeecamera.h
static const str_map effects[] = {
    { EFFECT_NONE,       CAMERA_EFFECT_OFF },
    { EFFECT_MONO,       CAMERA_EFFECT_MONO },
    { EFFECT_NEGATIVE,   CAMERA_EFFECT_NEGATIVE },
    { EFFECT_SOLARIZE,   CAMERA_EFFECT_SOLARIZE },
    { EFFECT_SEPIA,      CAMERA_EFFECT_SEPIA },
//    { EFFECT_POSTERIZE,  CAMERA_EFFECT_POSTERIZE },
    { EFFECT_WHITEBOARD, CAMERA_EFFECT_WHITEBOARD },
    { EFFECT_BLACKBOARD, CAMERA_EFFECT_BLACKBOARD },
//    { EFFECT_AQUA,       CAMERA_EFFECT_AQUA }
};

// from qcamera/common/camera.h
static const str_map autoexposure[] = {
    { AUTO_EXPOSURE_FRAME_AVG,  CAMERA_AEC_FRAME_AVERAGE },
    { AUTO_EXPOSURE_CENTER_WEIGHTED, CAMERA_AEC_CENTER_WEIGHTED },
    { AUTO_EXPOSURE_SPOT_METERING, CAMERA_AEC_SPOT_METERING }
};

// from qcamera/common/camera.h
static const str_map antibanding[] = {
    { ANTIBANDING_OFF,  CAMERA_ANTIBANDING_OFF },
    { ANTIBANDING_50HZ, CAMERA_ANTIBANDING_50HZ },
    { ANTIBANDING_60HZ, CAMERA_ANTIBANDING_60HZ },
    { ANTIBANDING_AUTO, CAMERA_ANTIBANDING_AUTO }
};
...
static const str_map flash[] = {
    { FLASH_MODE_OFF,  LED_MODE_OFF },
    { FLASH_MODE_AUTO, LED_MODE_AUTO },
    { FLASH_MODE_ON, LED_MODE_ON },
    { "torch", LED_MODE_ON }
};

// from mm-camera/common/camera.h.
static const str_map iso[] = {
    { ISO_AUTO,  CAMERA_ISO_AUTO},
    { ISO_HJR,   CAMERA_ISO_DEBLUR},
    { ISO_100,   CAMERA_ISO_100},
    { ISO_200,   CAMERA_ISO_200},
    { ISO_400,   CAMERA_ISO_400},
    { ISO_800,   CAMERA_ISO_800 }
};


#define DONT_CARE 0
static const str_map focus_modes[] = {
    { FOCUS_MODE_AUTO,     AF_MODE_AUTO},
    { FOCUS_MODE_INFINITY, DONT_CARE },
    { FOCUS_MODE_NORMAL,   AF_MODE_NORMAL },
    { FOCUS_MODE_MACRO,    AF_MODE_MACRO }
};

static const str_map lensshade[] = {
    { LENSSHADE_ENABLE, TRUE },
    { LENSSHADE_DISABLE, FALSE }
};
...
static const str_map picture_formats[] = {
        {PIXEL_FORMAT_JPEG, PICTURE_FORMAT_JPEG},
        {PIXEL_FORMAT_RAW, PICTURE_FORMAT_RAW}
};
...
{
    LOGD("cam_frame_flush_video: in n = %d\n", g_busy_frame_queue.num_of_frames);
    ...
    LOGD("cam_frame_flush_video: out n = %d\n", g_busy_frame_queue.num_of_frames);
    return ;
}
/*===========================================================================
...
return;
    ...
    LOGD(" Storing the current target type as %d ", mCurrentTarget );
    return;
}
...
void QualcommCameraHardware::initDefaultParameters()
{
    ...
    if (!parameter_string_initialized) {
		LOGD("%s : findSensorType()", __FUNCTION__);
        findSensorType();
		LOGD("%s : Setting default params...", __FUNCTION__);
		...
		LOGD("%s : end set default params", __FUNCTION__);
    }
    ...
    if((strcmp(mSensorInfo.name, "vx6953")) &&
		...
		(strcmp(mSensorInfo.name, "s5k3e2fx")) &&
        (strcmp(sensorType->name, "2mp"))){
        mParameters.set(
            KEY_SUPPORTED_PREVIEW_FRAME_RATES,
    ...
    mParameters.set(KEY_JPEG_QUALITY, "85"); // max quality
    mParameters.set(KEY_JPEG_THUMBNAIL_WIDTH,
                    THUMBNAIL_WIDTH_STR); // informative
    mParameters.set(KEY_JPEG_THUMBNAIL_HEIGHT,
                    THUMBNAIL_HEIGHT_STR); // informative
    mDimension.ui_thumbnail_width =
            thumbnail_sizes[DEFAULT_THUMBNAIL_SETTING].width;
    mDimension.ui_thumbnail_height =
            thumbnail_sizes[DEFAULT_THUMBNAIL_SETTING].height;
    mParameters.set(KEY_JPEG_THUMBNAIL_QUALITY, "90");

    mParameters.set(KEY_ANTIBANDING,
                    ANTIBANDING_OFF);
    mParameters.set(KEY_EFFECT,
                    EFFECT_NONE);
    mParameters.set(KEY_AUTO_EXPOSURE,
                    AUTO_EXPOSURE_FRAME_AVG);
    mParameters.set(KEY_WHITE_BALANCE,
                    WHITE_BALANCE_AUTO);
    mParameters.set(KEY_FOCUS_MODE,
                    FOCUS_MODE_AUTO);
    mParameters.set(KEY_SUPPORTED_PREVIEW_FORMATS,
                    "yuv420sp");

    mParameters.set(KEY_SUPPORTED_PREVIEW_SIZES,
                    preview_size_values.string());
    mParameters.set(KEY_SUPPORTED_PICTURE_SIZES,
                    picture_size_values.string());
    mParameters.set(KEY_SUPPORTED_ANTIBANDING,
                    antibanding_values);
    mParameters.set(KEY_SUPPORTED_EFFECTS, effect_values);
    mParameters.set(KEY_SUPPORTED_AUTO_EXPOSURE, autoexposure_values);
    mParameters.set(KEY_SUPPORTED_WHITE_BALANCE,
                    whitebalance_values);
    mParameters.set(KEY_SUPPORTED_FOCUS_MODES,
                    focus_mode_values);
    mParameters.set(KEY_SUPPORTED_PICTURE_FORMATS,
                    picture_format_values);

    if (mSensorInfo.flash_enabled) {
        mParameters.set(KEY_FLASH_MODE,
                        FLASH_MODE_OFF);
        mParameters.set(KEY_SUPPORTED_FLASH_MODES,
                        flash_values);
    }

    mParameters.set(KEY_MAX_SHARPNESS,
            CAMERA_MAX_SHARPNESS);
    mParameters.set(KEY_MAX_CONTRAST,
            CAMERA_MAX_CONTRAST);
    mParameters.set(KEY_MAX_SATURATION,
    ...
    mParameters.set(KEY_MAX_EXPOSURE_COMPENSATION,
            CAMERA_MAX_EXPOSURE_COMPENSATION);
    mParameters.set(KEY_MIN_EXPOSURE_COMPENSATION,
            CAMERA_MIN_EXPOSURE_COMPENSATION);
    mParameters.set(KEY_EXPOSURE_COMPENSATION_STEP,
    ...
    mParameters.set(KEY_PICTURE_FORMAT,
                    PIXEL_FORMAT_JPEG);

    mParameters.set(KEY_SHARPNESS,
                    CAMERA_DEF_SHARPNESS);
    mParameters.set(KEY_CONTRAST,
                    CAMERA_DEF_CONTRAST);
    mParameters.set(KEY_SATURATION,
                    CAMERA_DEF_SATURATION);
    mParameters.set(KEY_EXPOSURE_COMPENSATION,
                    CAMERA_DEF_EXPOSURE_COMPENSATION);

    mParameters.set(KEY_ISO_MODE,
                    ISO_AUTO);
    mParameters.set(KEY_LENSSHADE,
                    LENSSHADE_DISABLE);
    mParameters.set(KEY_SUPPORTED_ISO_MODES,
                    iso_values);
	LOGD("end mParamters stuff");
    if( (!strcmp(sensorType->name, "2mp")) ||
        (!strcmp(mSensorInfo.name, "vx6953")) ||
		(!strcmp(mSensorInfo.name, "ov5642")) ||
		(!strcmp(mSensorInfo.name, "s5k3e2fx")) ||
		(!strcmp(mSensorInfo.name, "VX6953")) ) {
        LOGD("Parameter Rolloff is not supported for this sensor");
    } else {
        mParameters.set(KEY_SUPPORTED_LENSSHADE_MODES,
                    lensshade_values);
    }
...
void QualcommCameraHardware::findSensorType(){
    ...
    bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_DIMENSION,
...
*/
	...
	LOGI("Open %s, fd %d", MSM_CAMERA_CONTROL, mCameraControlFd);
...
/*
    picture_sizes2 = LINK_default_sensor_get_snapshot_sizes(2);
    
        LOGD("size 1: %d x %d", picture_sizes2[0].width, picture_sizes2[0].height);
  */  
    LOGD("startCamera X");
    return true;
}
...
static bool native_stop_recording(int camfd)
{
    ...
    LOGD("in native_stop_recording returned %d", ret);
    return true;
}
/*===========================================================================
...
void QualcommCameraHardware::setGpsParameters() {
    ...
    //Set Latitude
    str = mParameters.get(KEY_GPS_LATITUDE);
    if(str != NULL) {
        setLatLon(EXIFTAGID_GPS_LATITUDE, str);
        //set Latitude Ref
        str = NULL;
        str = mParameters.get(KEY_GPS_LATITUDE_REF);
        if(str != NULL) {
    ...
    //set Longitude
    str = NULL;
    str = mParameters.get(KEY_GPS_LONGITUDE);
    if(str != NULL) {
        setLatLon(EXIFTAGID_GPS_LONGITUDE, str);
        //set Longitude Ref
        str = NULL;
        str = mParameters.get(KEY_GPS_LONGITUDE_REF);
        if(str != NULL) {
    ...
    //set Altitude
    str = NULL;
    str = mParameters.get(KEY_GPS_ALTITUDE);
    if(str != NULL) {
        ...
        addExifTag(EXIFTAGID_GPS_ALTITUDE, EXIF_RATIONAL, 1,
                    1, (void *)&altitude);
        //set AltitudeRef
        int ref = mParameters.getInt(KEY_GPS_ALTITUDE_REF);
        if( !(ref < 0 || ref > 1) )
...
bool QualcommCameraHardware::native_set_parm(int camfd,
    cam_ctrl_type type, uint16_t length, void *value)
{
    ...
    if (ioctl(camfd, MSM_CAM_IOCTL_CTRL_COMMAND, &ctrlCmd) < 0 ||
        ...
        LOGE("%s: error (%s): fd %d, type %d, length %d, status %d",
             ...
             camfd, type, length, ctrlCmd.status);
        return false;
    }
    return true;
}
...
#endif
    {
		  LOGD("%s %s", __FUNCTION__, "LINK_cam_frame");
        LINK_cam_frame(data);
    }
LOGD("%s mPreviewHeap.clear()", __FUNCTION__);
    mPreviewHeap.clear();
    ...
    while(true) {
        ...
        if(vframe != NULL) {
          ...
          frameCnt++;
			...
			camera_memory_t * cameraData = mMemoryCallback(-1, mPreviewHeap->mBufferSize, 1, mCallbackCookie);
			ssize_t  frameOffset;
			size_t  frameSize;
			mPreviewHeap->mBuffers[offset]->getMemory(&frameOffset, &frameSize);
            ...
            if(rcb != NULL && (msgEnabled & CAMERA_MSG_VIDEO_FRAME) ) {
                ...
                rcb(systemTime(), CAMERA_MSG_VIDEO_FRAME, cameraData, 0, rdata);
            }
#else
...
bool QualcommCameraHardware::initPreview()
{
    ...
    while (mInSnapshotMode) {
        LOGD("initPreview: waiting for snapshot mode to complete.");
        mInSnapshotModeWait.wait(mInSnapshotModeWaitLock);
        LOGD("initPreview: snapshot mode completed.");
    }
...
/*
		...
		new PmemPool("/dev/pmem_adsp",
			MemoryHeapBase::READ_ONLY | MemoryHeapBase::NO_CACHING,
			mCameraControlFd,
			MSM_PMEM_PREVIEW,//MSM_PMEM_OUTPUT2,//MSM_PMEM_OUTPUT2
			mPreviewFrameSize,
			1,
			mPreviewFrameSize,
			"postview");
    ...
    if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250) ) {
		...
		LOGD("%s : initrecord", __FUNCTION__);
        initRecord();
    }
    ...
    if (ret) {
        for (cnt = 0; cnt < kPreviewBufferCount; cnt++) {
            ...
            frames[cnt].path = MSM_FRAME_ENC;//OUTPUT_TYPE_P; //OUTPUT_TYPE_P; // MSM_FRAME_ENC;
        }
		LOGD("%s : camera_set_parm_dimension  ret=%d", __FUNCTION__, ret);
        mFrameThreadWaitLock.lock();
        pthread_attr_t attr;
...
bool QualcommCameraHardware::initRaw(bool initJpegHeap)
{
    ...
    LOGD("initRaw E: picture size=%dx%d", rawWidth, rawHeight);
    ...
    LOGD("Thumbnail Size Width %d Height %d",
            mDimension.ui_thumbnail_width,
            mDimension.ui_thumbnail_height);
...
status_t QualcommCameraHardware::startPreviewInternal()
{
...

    {
        ...
        if(( mCurrentTarget != TARGET_MSM7630 ) && (mCurrentTarget != TARGET_QSD8250))
		{	
			LOGD("%s : native_start_preview called", __FUNCTION__);
	...
	LOGD("%s mCameraRunning?", __FUNCTION__);
    if(!mCameraRunning) {
        deinitPreview();
        mPreviewInitialized = false;
        //mOverlay = NULL;
	...
	LOGD("%s native_get_maxzoom", __FUNCTION__);
	// TODO: fix below
...
void QualcommCameraHardware::runAutoFocus()
{
    ...
    if ((mParameters.get(KEY_FOCUS_MODE) == 0)
           || (strcmp(mParameters.get(KEY_FOCUS_MODE),
               FOCUS_MODE_INFINITY) == 0)) {
        goto done;
    }
    ...
    afMode = (isp3a_af_mode_t)attr_lookup(focus_modes,
                                ...
                                mParameters.get(KEY_FOCUS_MODE));
...
{
    LOGD("cancelAutoFocusInternal E");

    if(!sensorType->hasAutoFocusSupport){
        LOGD("cancelAutoFocusInternal X");
        return NO_ERROR;
    }

#if 0
    if (mAutoFocusFd < 0) {
        LOGD("cancelAutoFocusInternal X: not in progress");
        return NO_ERROR;
    }
#endif
    ...
    if(err == NO_ERROR) {
        ...
        LOGD("As Auto Focus is not in progress, Cancel Auto Focus "
                "is ignored");
        mAfLock.unlock();
    }
    else {
        //AF is in Progess, So cancel it
        LOGD("Lock busy...cancel AF");
    ...
    LOGD("cancelAutoFocusInternal X: %d", rc);
    return rc;
}
...
void QualcommCameraHardware::runSnapshotThread(void *data)
{
    ...
    while (mJpegThreadRunning) {
        LOGD("runSnapshotThread: waiting for jpeg thread to complete.");
        mJpegThreadWait.wait(mJpegThreadWaitLock);
        LOGD("runSnapshotThread: jpeg thread completed.");
    }
    mJpegThreadWaitLock.unlock();
    //clear the resources
    LINK_jpeg_encoder_join();
...
{
    LOGD("takePicture(%d)", mMsgEnabled);
    Mutex::Autolock l(&mLock);
...
status_t QualcommCameraHardware::setParameters(const CameraParameters& params)
{
    ...
    if ((rc = setPreviewSize(params))) final_rc = rc; LOGD("setPreviewSize");
    if ((rc = setPreviewFrameRate(params))) final_rc = rc;LOGD("setPreviewFrameRate");
    if ((rc = setPictureSize(params)))  final_rc = rc;LOGD("setPictureSize");
    if ((rc = setJpegQuality(params)))  final_rc = rc;LOGD("setJpegQuality");
    if ((rc = setAntibanding(params)))  final_rc = rc;LOGD("setAntibanding");
    if ((rc = setAutoExposure(params))) final_rc = rc;LOGD("setAutoExposure");
    if ((rc = setWhiteBalance(params))) final_rc = rc;LOGD("setWhiteBalance");
    if ((rc = setEffect(params)))       final_rc = rc;LOGD("setEffect");
    if ((rc = setFlash(params)))        final_rc = rc;LOGD("setFlash");
    if ((rc = setGpsLocation(params)))  final_rc = rc;LOGD("setGpsLocation");
    if ((rc = setRotation(params)))     final_rc = rc;LOGD("setRotation"); 
    //if ((rc = setZoom(params)))         final_rc = rc;LOGD("setZoom");
    if ((rc = setFocusMode(params)))    final_rc = rc;LOGD("setFocusMode");
    if ((rc = setOrientation(params)))  final_rc = rc;LOGD("setOrientation");
    if ((rc = setBrightness(params)))   final_rc = rc;LOGD("setBrightness");
    if ((rc = setExposureCompensation(params)))   final_rc = rc;LOGD("setExposureCompensation");
    //if ((rc = setLensshadeValue(params)))  final_rc = rc;LOGD("setLensshadeValue");
    if ((rc = setISOValue(params)))  final_rc = rc;LOGD("setISOValue");
    if ((rc = setPictureFormat(params))) final_rc = rc;LOGD("setPictureFormat");
    if ((rc = setSharpness(params)))    final_rc = rc;LOGD("setSharpness");
    if ((rc = setContrast(params)))     final_rc = rc;LOGD("setContrast");
    if ((rc = setSaturation(params)))   final_rc = rc;LOGD("setSaturation");
...
sp<CameraHardwareInterface> QualcommCameraHardware::createInstance()
{
    ...
    while (singleton_releasing) {
        if((singleton_releasing_start_time != 0) &&
            ...
            LOGD("in createinstance system time is %lld %lld %lld ",
                    systemTime(), singleton_releasing_start_time, SINGLETON_RELEASING_WAIT_TIME);
            singleton_lock.unlock();
	...
	LOGD("About to set default params");
...
void QualcommCameraHardware::receivePreviewFrame(struct msm_frame *frame)
{
	...
	// Get the memory we want
	camera_memory_t * cameraData = mMemoryCallback(mPreviewHeap->mHeap->getHeapID(), mPreviewHeap->mBufferSize, 1, mCallbackCookie);
	/*ssize_t  frameOffset;
	size_t  frameSize;*/
	//mPreviewHeap->mBuffers[offset]->getMemory(&frameOffset, &frameSize);
		//		 memcpy(cameraData->data,   (void*) mJpegHeap->mHeap->getBase(), mJpegSize);
	 //memcpy(cameraData->data,   (void*) frameOffset, frameSize);
	 memcpy(cameraData->data,   (void*) offset_addr, mPreviewHeap->mBufferSize);
    ...
    if (pcb != NULL && (msgEnabled & CAMERA_MSG_PREVIEW_FRAME))
        pcb(CAMERA_MSG_PREVIEW_FRAME, cameraData, 0 ,metaData, pdata);

    // If output  is NOT enabled (targets otherthan 7x30 currently..)
    if( (mCurrentTarget != TARGET_MSM7630 ) &&  (mCurrentTarget != TARGET_QSD8250)) {
        if(rcb != NULL && (msgEnabled & CAMERA_MSG_VIDEO_FRAME)) {
            rcb(systemTime(), CAMERA_MSG_VIDEO_FRAME, cameraData, 0 , pdata);
...
bool QualcommCameraHardware::initRecord()
{
    ...
    mRecordHeap = new PmemPool(pmem_region,
                               MemoryHeapBase::READ_ONLY | MemoryHeapBase::NO_CACHING,
                                mCameraControlFd,
                                MSM_PMEM_OUTPUT2,//,16,//MSM_PMEM_VIDEO,//MSM_PMEM_RAW_MAINIMG, //MSM_PMEM_OUTPUT2, //MSM_PMEM_VIDEO
                                mRecordFrameSize,
                                kRecordBufferCount,
                                mRecordFrameSize,
                                "record");
...
void QualcommCameraHardware::receiveRawSnapshot(){
    ...
    if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
        ...
        notifyShutter(&mCrop);
		// Get the memory we want
		camera_memory_t * cameraData = mMemoryCallback(-1, mRawSnapShotPmemHeap->mBufferSize, 1, mCallbackCookie);
		ssize_t  frameOffset;
		size_t  frameSize;
		 mRawSnapShotPmemHeap->mBuffers[0]->getMemory(&frameOffset, &frameSize);
...
*/
	...
	//memcpy(tmpMem + mJpegSize, buff_ptr, buff_size);
    mJpegSize += buff_size;
...
status_t QualcommCameraHardware::setPreviewSize(const CameraParameters& params)
{
    ...
    LOGD("requested preview size %d x %d", width, height);
...
{
    if((!strcmp(mSensorInfo.name, "vx6953")) || (!strcmp(mSensorInfo.name, "VX6953")) || (!strcmp(sensorType->name, "2mp")) /*|| (!strcmp(mSensorInfo.name, "s5k3e2fx")) */)
	{       
		LOGD("set fps is not supported for this sensor");
        return NO_ERROR;
    }
    uint16_t previousFps = (uint16_t)mParameters.getPreviewFrameRate();
    uint16_t fps = (uint16_t)params.getPreviewFrameRate();
    LOGD("requested preview frame rate  is %u", fps);

    if(mInitialized && (fps == previousFps)){
        LOGD("fps same as previous fps");
        return NO_ERROR;
    }

    if(MINIMUM_FPS <= fps && fps <=MAXIMUM_FPS){
        mParameters.setPreviewFrameRate(fps);
        bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_FPS,
                sizeof(fps), (void *)&fps);
...
status_t QualcommCameraHardware::setPictureSize(const CameraParameters& params)
{
    ...
    LOGD("requested picture size %d x %d", width, height);
...
status_t QualcommCameraHardware::setJpegQuality(const CameraParameters& params) {
    ...
    if (quality > 0 && quality <= 100) {
        mParameters.set(KEY_JPEG_QUALITY, quality);
    } else {
    ...
    if (quality > 0 && quality <= 100) {
        mParameters.set(KEY_JPEG_THUMBNAIL_QUALITY, quality);
    } else {
...
status_t QualcommCameraHardware::setEffect(const CameraParameters& params)
{
    ...
    const char *str_wb = mParameters.get(KEY_WHITE_BALANCE);
    ...
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
           ...
           || (value == CAMERA_EFFECT_AQUA) || (value == CAMERA_EFFECT_SEPIA))
               ...
               mParameters.set(KEY_EFFECT, str);
               bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_EFFECT, sizeof(value),
                                           (void *)&value);
               return ret ? NO_ERROR : UNKNOWN_ERROR;
          }
        }
...
status_t QualcommCameraHardware::setAutoExposure(const CameraParameters& params)
{
    ...
    const char *str = params.get(KEY_AUTO_EXPOSURE);
    if (str != NULL) {
        int32_t value = attr_lookup(autoexposure, sizeof(autoexposure) / sizeof(str_map), str);
        if (value != NOT_FOUND) {
            mParameters.set(KEY_AUTO_EXPOSURE, str);
            bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_EXPOSURE, sizeof(value),
                                       (void *)&value);
            return ret ? NO_ERROR : UNKNOWN_ERROR;
        }
    }
...
{
    if(!strcmp(mSensorInfo.name, "2mp") || !strcmp(mSensorInfo.name, "s5k3e2fx")) {
        LOGE("Sharpness not supported for this sensor");
        return NO_ERROR;
    }
    int sharpness = params.getInt(KEY_SHARPNESS);
    if((sharpness < CAMERA_MIN_SHARPNESS
            || sharpness > CAMERA_MAX_SHARPNESS))
        return UNKNOWN_ERROR;

    LOGD("setting sharpness %d", sharpness);
    mParameters.set(KEY_SHARPNESS, sharpness);
    bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_SHARPNESS, sizeof(sharpness),
                               (void *)&sharpness);
    return ret ? NO_ERROR : UNKNOWN_ERROR;
}
...
status_t QualcommCameraHardware::setContrast(const CameraParameters& params)
{
    ...
    int contrast = params.getInt(KEY_CONTRAST);
    if((contrast < CAMERA_MIN_CONTRAST)
            || (contrast > CAMERA_MAX_CONTRAST))
        return UNKNOWN_ERROR;

    LOGD("setting contrast %d", contrast);
    mParameters.set(KEY_CONTRAST, contrast);
    bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_CONTRAST, sizeof(contrast),
                               (void *)&contrast);
    return ret ? NO_ERROR : UNKNOWN_ERROR;
}
...
status_t QualcommCameraHardware::setSaturation(const CameraParameters& params)
{
	...
	int saturation = params.getInt(KEY_SATURATION);
	if((saturation < CAMERA_MIN_SATURATION)
		|| (saturation > CAMERA_MAX_SATURATION))
	    return UNKNOWN_ERROR;

	LOGD("setting saturation %d", saturation);
	mParameters.set(KEY_SATURATION, saturation);
	bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_SATURATION, sizeof(saturation),
		(void *)&saturation);
	return ret ? NO_ERROR : UNKNOWN_ERROR;
    } else {
...
status_t QualcommCameraHardware::setBrightness(const CameraParameters& params) {
        ...
        if (mBrightness !=  brightness) {
            ...
            bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_BRIGHTNESS, sizeof(mBrightness),
                                       (void *)&mBrightness);
            return ret ? NO_ERROR : UNKNOWN_ERROR;
        } else {
            return NO_ERROR;
...
status_t QualcommCameraHardware::setExposureCompensation(const CameraParameters& params) {
	...
	mParameters.set(KEY_EXPOSURE_COMPENSATION, expcomp);
	...
	if(!strcmp(sensorType->name, "3mp"))
	  ...
	  expcomp+=2;
        ...
        bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_EXPOSURE_COMPENSATION, sizeof(expcomp),
                                       (void *)&expcomp);
...
status_t QualcommCameraHardware::setWhiteBalance(const CameraParameters& params)
{
    ...
    const char *str_effect = mParameters.get(KEY_EFFECT);
    ...
    && (value_effect != CAMERA_EFFECT_AQUA) && (value_effect != CAMERA_EFFECT_SEPIA)) {
        ...
        if (str != NULL) {
            ...
            if (value != NOT_FOUND) {
                mParameters.set(KEY_WHITE_BALANCE, str);
                bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_WB, sizeof(value),
                                           (void *)&value);
                return ret ? NO_ERROR : UNKNOWN_ERROR;
            }
        }
...
{
    if (!mSensorInfo.flash_enabled) {
        LOGD("%s: flash not supported", __FUNCTION__);
        return NO_ERROR;
    }
    ...
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            mParameters.set(KEY_FLASH_MODE, str);
            bool ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_LED_MODE,
...
status_t QualcommCameraHardware::setAntibanding(const CameraParameters& params)
{
    ...
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            ...
            mParameters.set(KEY_ANTIBANDING, str);
            bool ret;
            if (temp == CAMERA_ANTIBANDING_AUTO) {
                ret = native_set_parm(mCameraControlFd, CAMERA_ENABLE_AFD,
                            0, NULL);
            } else {
                ret = native_set_parm(mCameraControlFd, CAMERA_SET_PARM_ANTIBANDING,
...
{
    if( (!strcmp(sensorType->name, "2mp")) ||  (!strcmp(mSensorInfo.name, "vx6953")) || (!strcmp(mSensorInfo.name, "ov5642")) || (!strcmp(mSensorInfo.name, "VX6953"))  /*|| (!strcmp(mSensorInfo.name, "s5k3e2fx"))*/ ) 
	{
        LOGD("Parameter Rolloff is not supported for this sensor");
        return NO_ERROR;
    }
    const char *str = params.get(KEY_LENSSHADE);
    if (str != NULL) {
        int value = attr_lookup(lensshade,
                                    sizeof(lensshade) / sizeof(str_map), str);
        if (value != NOT_FOUND) {
            int8_t temp = (int8_t)value;
            mParameters.set(KEY_LENSSHADE, str);
            native_set_parm(mCameraControlFd, CAMERA_SET_PARM_ROLLOFF, sizeof(int8_t), (void *)&temp);
            return NO_ERROR;
        }
    }
...
status_t  QualcommCameraHardware::setISOValue(const CameraParameters& params) {
    int8_t temp_hjr;
    const char *str = params.get(KEY_ISO_MODE);
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            ...
            if (value == CAMERA_ISO_DEBLUR) {
               temp_hjr = true;
               native_set_parm(mCameraControlFd, CAMERA_SET_PARM_HJR, sizeof(int8_t), (void*)&temp_hjr);
               mHJR = value;
            }
            else {
			...
			//tmp, REMOVE!
			str = "ISO100";
            mParameters.set(KEY_ISO_MODE, str);
            native_set_parm(mCameraControlFd, CAMERA_SET_PARM_ISO, sizeof(camera_iso_mode_type), (void *)&temp);
            return NO_ERROR;
        }
    }
...
status_t QualcommCameraHardware::setGpsLocation(const CameraParameters& params)
{
    ...
    if (latitude) {
        mParameters.set(KEY_GPS_LATITUDE, latitude);
    }

    const char *latitudeRef = params.get(KEY_GPS_LATITUDE_REF);
    if (latitudeRef) {
        mParameters.set(KEY_GPS_LATITUDE_REF, latitudeRef);
    }

    const char *longitude = params.get(KEY_GPS_LONGITUDE);
    if (longitude) {
        mParameters.set(KEY_GPS_LONGITUDE, longitude);
    }

    const char *longitudeRef = params.get(KEY_GPS_LONGITUDE_REF);
    if (longitudeRef) {
        mParameters.set(KEY_GPS_LONGITUDE_REF, longitudeRef);
    }

    const char *altitudeRef = params.get(KEY_GPS_ALTITUDE_REF);
    if (altitudeRef) {
        mParameters.set(KEY_GPS_ALTITUDE_REF, altitudeRef);
    }

    const char *altitude = params.get(KEY_GPS_ALTITUDE);
    if (altitude) {
        mParameters.set(KEY_GPS_ALTITUDE, altitude);
    }

    const char *status = params.get(KEY_GPS_STATUS);
    if (status) {
        mParameters.set(KEY_GPS_STATUS, status);
    }

    const char *dateTime = params.get(KEY_EXIF_DATETIME);
    if (dateTime) {
        mParameters.set(KEY_EXIF_DATETIME, dateTime);
    }

    const char *timestamp = params.get(KEY_GPS_TIMESTAMP);
    if (timestamp) {
        mParameters.set(KEY_GPS_TIMESTAMP, timestamp);
    }
    return NO_ERROR;
}
...
status_t QualcommCameraHardware::setRotation(const CameraParameters& params)
{
    ...
    if (rotation != NOT_FOUND) {
        if (rotation == 0 || rotation == 90 || rotation == 180
            || rotation == 270) {
          mParameters.set(KEY_ROTATION, rotation);
        } else {
...
status_t QualcommCameraHardware::setFocusMode(const CameraParameters& params)
{
    ...
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            mParameters.set(KEY_FOCUS_MODE, str);
...
{
    const char * str = params.get(KEY_PICTURE_FORMAT);

    if(str != NULL){
        int32_t value = attr_lookup(picture_formats,
                                    sizeof(picture_formats) / sizeof(str_map), str);
        if(value != NOT_FOUND){
            mParameters.set(KEY_PICTURE_FORMAT, str);
        } else {
    ...
    mCameraControlFd(dup(camera_control_fd))
{
    ...
    if (pmemHeap->getHeapID() >= 0) {
        ...
        if( (strcmp("postview", mName) != 0) ){
			...
			//if(strcmp("preview", mName) > -1) num_buf = 4;
            LOGD("num_buffers = %d", num_buf);
            for (int cnt = 0; cnt < num_buf; ++cnt) {
                int active = 1;
                if(pmem_type == MSM_PMEM_VIDEO ){  //was MSM_PMEM_VIDEO
                     active = (cnt<ACTIVE_VIDEO_BUFFERS);
                     LOGD(" pmempool creating video buffers : active %d ", active);
                }
                else if (pmem_type == MSM_PMEM_PREVIEW){ // was MSM_PMEM_PREVIEW
                     active = (cnt < (num_buf-1));
                }
				LOGD("register_buf %d %d %d %d %d %d %d %d", mCameraControlFd, mBufferSize, mFrameSize,  mHeap->getHeapID(),  mAlignedBufferSize * cnt, (uint8_t *)mHeap->base() + mAlignedBufferSize * cnt, pmem_type, active);
                         ...
                         bool register_buffer)
{
	...
	//pmemBuf.type	= MSM_PMEM_OUTPUT2; //MSM_PMEM_MAINIMG; //MUST BE one of:http://os1a.cs.columbia.edu/lxr/source/drivers/staging/dream/camera/msm_camera.c?a=x86#1392 ... patch is simple or bypass : https://www.codeaurora.org/gitweb/quic/le/?p=kernel/msm.git;a=commitdiff_plain;h=e0a1b78090a21c0313f6ef1223f075663e017d11
    ...
    LOGD("register_buf: camfd = %d, reg = %d buffer = %p can_write = %d",
...
/*void QualcommCameraHardware::setCallbacks(notify_callback notify_cb,
                             ...
                             void* user)
							 */
void QualcommCameraHardware::setCallbacks(struct camera_device * device,
...
bool QualcommCameraHardware::useOverlay(void)
{
	...
	return false; // TODO: fix overlay for ICS
    //return mUseOverlay;
}
/*
...
void QualcommCameraHardware::receive_camframetimeout(void) {
    ...
    mNotifyCallback(CAMERA_MSG_ERROR, CAMERA_ERROR_UKNOWN, 0,  mCallbackCookie);
